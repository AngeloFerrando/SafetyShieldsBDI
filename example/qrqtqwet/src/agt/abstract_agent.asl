...
+!plan1 : count(100) & .intention(I, _, Stack, current) & (ids(I, IDs) | IDs=[]) & .length(Stack, Depth) & .concat("shield1_id0_", Depth, IntIDD) & not(.member(IntIDD, IDs)) <- !push_id(I, IntDD); ids(I, CurrentlyActShields); add_shield(I, "shield1", "G(add_belief(belief4) -> X(action(action3)))"); !get_count(I, Count, 1); !pop_count(I, Count, ThisShieldId); remove_shield(I, ThisShieldId).
+!plan1 : count(C) & .intention(I, _, Stack, current) & (ids(I, IDs) | IDs=[]) & .length(Stack, Depth) & .concat("shield1_id1_", Depth, IntIDD) & not(.member(IntIDD, IDs)) <- !push_id(I, IntDD); ids(I, CurrentlyActShields); add_shield(I, "shield1", "G(add_belief(belief4) -> X(action(action3)))"); update_shield(I, CurrentlyActShields, "remove_belief(count(_))"); -count(_); update_shield(I, CurrentlyActShields, "add_belief(count(C+1))"); +count(C+1); update_shield(I, CurrentlyActShields, "add_belief(belief5)"); +belief5; update_shield(I, CurrentlyActShields, "goal(plan2)"); !plan2; update_shield(I, CurrentlyActShields, "action(action3)"); action3; update_shield(I, CurrentlyActShields, "goal(plan1)"); !plan1; !get_count(I, Count, 1); !pop_count(I, Count, ThisShieldId); remove_shield(I, ThisShieldId).
-!plan1 : .intention(I, _, Stack, current) & violated(I, "shield1", Cmds) & (count(I, Count) | Count = 1) <- -count(I, Count); +count(I, Count+1); !toTerms(Cmds, TCmds); !restore(TCmds); !plan1.
-!plan1 : .intention(I, _, Stack, current) <- !pop_count(I, Count, ThisShieldId); remove_shield(I, ThisShieldId).



+!plan2 : true & .intention(I, _, Stack, current) & (ids(I, IDs) | IDs=[]) & .length(Stack, Depth) & .concat("shield2_id2_", Depth, IntIDD) & not(.member(IntIDD, IDs)) <- !push_id(I, IntDD); ids(I, CurrentlyActShields); add_shield(I, "shield2", "G(add_belief(belief4) -> X(action(action3)))"); update_shield(I, CurrentlyActShields, "action(action4)"); action4; !get_count(I, Count, 1); !pop_count(I, Count, ThisShieldId); remove_shield(I, ThisShieldId).
-!plan2 : .intention(I, _, Stack, current) & violated(I, "shield2", Cmds) & (count(I, Count) | Count = 1) <- -count(I, Count); +count(I, Count+1); !toTerms(Cmds, TCmds); !restore(TCmds); !plan2.
-!plan2 : .intention(I, _, Stack, current) <- !pop_count(I, Count, ThisShieldId); remove_shield(I, ThisShieldId).
